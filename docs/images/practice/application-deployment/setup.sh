...
OS=$(uname)
TOKEN="212d3a25617b2265223a333630302c226b223a226e6f6465222c226e223a226e6f64652d31222c226e73223a2262616574796c2d636c6f7564222c227473223a313539333730303433337d"
RESOURCE_URL="https://106.12.34.129:31601"
SUDO=sudo
...
check_and_install_baetyl() {
  ...
  if [ ! -z "$KUBE_MASTER_NODE_NAME" ]; then
    ...
    kube_apply "$RESOURCE_URL/v1/active/baetyl-init.yml?token=$TOKEN&node=$KUBE_MASTER_NODE_NAME"
  else
...
check_and_get_metrics() {
  ...
  if [ -z "$METRICS" ]; then
    kube_apply "$RESOURCE_URL/v1/active/metrics.yml"
  fi
}
...
check_and_get_storage() {
  ...
  if [ -z "$PROVISIONER" ]; then
    kube_apply "$RESOURCE_URL/v1/active/local-path-storage.yml"
  fi
}
