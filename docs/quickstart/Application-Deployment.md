...
# 功能简介
应用是用于定义在一个或一组边缘节点上的部署任务，会将**服务配置**及**存储卷配置**下发到匹配的目标设备上，并进行部署。应用根据边缘节点的标签来匹配目标设备，每一次应用信息的更改都会即时的下发到目标设备，并进行服务重部署及配置的更新。
...
在实例详情页的左侧菜单中点击**IOT边缘-应用部署**，您将看到如下界面
...
设置**容器服务**的标签，点击标签旁边**书写图标**弹出添加标签界面

![06-create-container-app-label.png](../images/quickstart/application-deployment/06-create-container-app-label.png)
...
注意：应用修改后最新的应用配置会及时的下发到匹配的边缘节点，并进行更新。
